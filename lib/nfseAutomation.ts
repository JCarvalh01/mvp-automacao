...
function formatarDataParaPortal(dataIso: string) {
  if (!dataIso) return "";
...
function normalizarMunicipioParaPortal(valor: string) {
  return String(valor || "")
    .replace(/\s*-\s*/g, "/")
    .replace(/\s*\/\s*/g, "/")
    .replace(/\s+/g, " ")
...
function extrairNomeMunicipio(valor: string) {
  const normalizado = normalizarMunicipioParaPortal(valor);
  return normalizado.split("/")[0]?.trim() || normalizado;
}

function extrairUfMunicipio(valor: string) {
  const normalizado = normalizarMunicipioParaPortal(valor);
  const partes = normalizado.split("/");
  return (partes[1] || "").trim().toLowerCase();
}

function formatarValorParaDigitacaoNoPortal(valor: number | string) {
...
function erroEhTransitorio(erro: unknown): boolean {
  const msg = String(
...
async function clicarComFallback(page: Page, selectors: string[], nome: string) {
  for (const selector of selectors) {
    try {
      const locator = page.locator(selector).first();
      const count = await locator.count();
...
async function clicarSeExistir(page: Page, selectors: string[], nome: string) {
  for (const selector of selectors) {
    try {
      const locator = page.locator(selector).first();
      const count = await locator.count();
...
async function obterInputSelect2Visivel(page: Page) {
...
async function abrirSelect2ECapturarBusca(
  page: Page,
  campo: Locator,
  nomeCampo: string
) {
...
async function abrirTelaPessoas(page: Page) {
...
async function preencherDataCompetencia(page: Page, dataIso: string) {
...
async function clicarEspacoEmBrancoParaCarregarEmitente(page: Page) {
  const tentativas = [
...
async function esperarEmitentePreenchido(page: Page, cnpj: string) {
  const candidatos = [
    `text="${cnpj}"`,
...
async function selecionarBrasilTomador(page: Page) {
  const seletores = [
...
async function preencherDocumentoTomador(page: Page, documentoTomador: string) {
  await preencherCampoComFallback(
    page,
    [
...
async function selecionarMunicipioPrestacao(page: Page, municipioCompleto: string) {
  ...
  let melhorOpcao: Locator | null = null;
...
async function selecionarCodigoTributacao(page: Page, taxCode: string) {
...
async function marcarOpcaoNao(page: Page) {
  const seletores = [
...
async function preencherDescricaoServico(page: Page, descricao: string) {
  await preencherCampoComFallback(
    page,
    [
...
async function baixarArquivoPorBotao(
  page: Page,
  selectors: string[],
  nomeLogico: string,
  nomeArquivo: string
...
async function concluirEmissao(page: Page): Promise<EmitirNotaResult> {
